...
;==============================================================================
    ...
    #include "univ_3-1-1-0-rev0.inc"                         ;project variables
INCLUDEDFILES   code    
...
Main:
    ...
    ;initiate ports                         ;default all pins set as analog (portA) or digital (portB,C) inputs 
        ;PORT A
        movlb   0xF                         ;bank 16
        movlw   b'11100011'                 ;0 -digital input, 1 -analog input (AN0 - AN4)
        movwf   ANCON0
        clrf    LATA                        ;all low
        movlw   b'11111111'                 ;all inputs
        movwf   TRISA        
        ;PORT B
        movlw   b'11111000'                 ;0 -digital input, 1 -analog input (AN8 - AN10)
        movwf   ANCON1
        clrf    LATB                        ;all low
...
TransmitTemp
		...
		setf	TXFIFOIN4
		setf	TXFIFOIN5
		;TXBD2 - TXBD7 are already changed
		call	WriteToCanTxFIFO
    ;node can respond to its own message
        bcf     INTCON,GIEL                 ;disable low priority intr to make sure RXFIFO buffer is not overwritten
        call    Copy_TXFIFOIN_RXFIFOIN
